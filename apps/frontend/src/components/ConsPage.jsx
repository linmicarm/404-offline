...
export default function ConsPage({ setCurrentPage, setEditingCon, showModal }) {
  ...
  return (
    <div className="page">
      <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <button className="btn-primary" onClick={() => { setEditingCon(null); setCurrentPage("con-form"); }}>
          + Add con
        </button>
      </div>
            ...
            {upcoming.map((con) => {
              ...
              return (
                <div key={con.id} className="card" style={{ display: "flex", alignItems: "stretch", padding: 0, overflow: "hidden" }}>
                  <div style={{ background: "var(--surface2)", borderRight: "1.5px solid var(--border)", padding: "1.25rem", minWidth: "100px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "4px" }}>
                    <div className="mono" style={{ fontSize: "11px", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "1px" }}>
                      {new Date(con.start_date).toLocaleString("default", { month: "short" })}
                    </div>
                    <div style={{ fontSize: "28px", fontWeight: "800", color: "var(--ink)", lineHeight: 1 }}>
                      {new Date(con.start_date).getDate()}
                    </div>
                    <div className="mono" style={{ fontSize: "9px", color: "var(--ink-3)" }}>
                  ...
                  <div style={{ flex: 1, padding: "1.125rem" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "6px" }}>
                      ...
                      {countdown && (
                        <span className="tag tag-peach" style={{ whiteSpace: "nowrap", marginLeft: "8px" }}>{countdown}</span>
                      )}
                    </div>
