...
export default function SideQuestDetail({ sideQuest, setCurrentPage, setEditingSideQuest, showModal, showToast, setSelectedSpawnPoint, setSelectedSideQuest }) {
  ...
  return (
    <div>
      <div style={{ minHeight: "380px", background: quest.image_url ? `url(${quest.image_url}) center/cover` : gradient, position: "relative", display: "flex", alignItems: "flex-end" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, padding: "2rem 2.5rem", maxWidth: "1400px", width: "100%" }}>
          <button onClick={() => setCurrentPage("side-quests")} style={{ fontFamily: "var(--font-mono)", fontSize: "11px", background: "rgba(255,252,247,0.12)", backdropFilter: "blur(4px)", border: "1px solid rgba(255,252,247,0.2)", color: "#FFFCF7", padding: "6px 14px", borderRadius: "100px", cursor: "pointer", marginBottom: "1.25rem", display: "inline-block" }}>
            ← Back to side quests
          </button>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: "10px", letterSpacing: "3px", textTransform: "uppercase", color: "rgba(255,252,247,0.5)", marginBottom: "8px" }}>{quest.category}</div>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "52px", fontWeight: "400", color: "#FFFCF7", lineHeight: 1.05, letterSpacing: "-0.5px", marginBottom: "1rem" }}>{quest.name}</h1>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "1.25rem" }}>
            ...
            {quest.is_beginner_friendly && <span style={{ fontFamily: "var(--font-mono)", fontSize: "11px", fontWeight: "700", background: "rgba(133,201,160,0.3)", color: "#A8E4BC", border: "1px solid rgba(133,201,160,0.5)", padding: "4px 12px", borderRadius: "100px" }}>Beginner ok</span>}
            {quest.is_recurring && quest.recurrence && <span style={{ fontFamily: "var(--font-mono)", fontSize: "11px", fontWeight: "700", background: "rgba(255,170,127,0.3)", color: "#FFCBA4", border: "1px solid rgba(255,170,127,0.5)", padding: "4px 12px", borderRadius: "100px" }}>{RECURRENCE_LABELS[quest.recurrence]}</span>}
          </div>

          <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
            <button onClick={handleGoing} disabled={goingLoading} style={{ fontFamily: "var(--font-mono)", fontSize: "13px", fontWeight: "700", background: isGoing ? "var(--sage)" : "var(--peach)", color: isGoing ? "#1A4D32" : "#6B3218", border: "none", padding: "12px 24px", borderRadius: "100px", cursor: goingLoading ? "default" : "pointer", transition: "all 0.15s" }}>
              {isGoing ? `✓ Going (${quest.going_count})` : `I'm going — ${quest.going_count} interested`}
            </button>
            <button onClick={() => downloadICS(quest)} style={{ fontFamily: "var(--font-mono)", fontSize: "11px", background: "rgba(255,252,247,0.12)", color: "rgba(255,252,247,0.7)", border: "1px solid rgba(255,252,247,0.2)", padding: "10px 18px", borderRadius: "100px", cursor: "pointer" }}>
              📅 Add to calendar
            </button>
            <button onClick={handleShare} style={{ fontFamily: "var(--font-mono)", fontSize: "11px", background: "rgba(255,252,247,0.12)", color: copied ? "#A8E4BC" : "rgba(255,252,247,0.7)", border: `1px solid ${copied ? "rgba(133,201,160,0.5)" : "rgba(255,252,247,0.2)"}`, padding: "10px 18px", borderRadius: "100px", cursor: "pointer", transition: "all 0.2s" }}>
              {copied ? "✓ Copied!" : "Share"}
            </button>
          </div>
        </div>
      ...
      <div className="page">
        <div style={{ marginBottom: "2rem" }}>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: "17px", color: "var(--ink-2)", lineHeight: 1.8 }}>{quest.description}</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginBottom: "2rem" }}>
          ...
          {quest.spawn_point && (
            <div className="card" style={{ cursor: "pointer", borderColor: "var(--peach)" }} onClick={() => { setSelectedSpawnPoint(quest.spawn_point); setCurrentPage("spawn-point-detail"); }}>
          ...
          <>
            ...
            <div className="grid-2" style={{ marginBottom: "2rem" }}>
              {relatedQuests.slice(0, 4).map((q) => (
                <SideQuestCard key={q.id} sideQuest={{ ...q, spawn_point: quest.spawn_point }} onClick={(clicked) => { setSelectedSideQuest(clicked); setCurrentPage("side-quest-detail"); }} />
              ))}
            </div>
          </>
        )}
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "2rem", paddingTop: "1.5rem", borderTop: "1px solid var(--border)" }}>
          <button className="btn-secondary" onClick={() => { setEditingSideQuest(quest); setCurrentPage("side-quest-form"); }}>Edit side quest</button>
          <button className="btn-danger" onClick={handleDelete}>Delete side quest</button>
        </div>
      </div>
    </div>
  );
